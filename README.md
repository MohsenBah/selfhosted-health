...
|---|---|---|
| **`.env`** (repo root) | Docker Compose (ports, DB passwords for *containers*) | `OW_DB_PASSWORD`, `WGER_DB_PASSWORD`. `GRAFANA_*` only if you use `--profile grafana` / `full` (safe to omit with an external Grafana). For LAN demos, set `OW_PUBLIC_API_URL` to `http://<host-ip>:…` before building. |
...
docker compose --profile wearables --profile gym up -d --build   # lab: no local Grafana
docker compose --profile full up -d --build
```
